...
|/author|.recorder<br/>&<br/>**[Provenance](http://hl7.org/fhir/us/core/STU6.1/StructureDefinition-us-core-provenance.html)**|**Constraint:** Only map last CDA author to FHIR recorder<br/>[Guidance on CDA ↔ FHIR Provenance](mappingGuidance.html#cda--fhir-provenance)|
...
|**[Allergy Status](https://hl7.org/cda/us/ccda/3.0.0/StructureDefinition-AllergyStatusObservation.html)**<br/>/entryRelationship/observation[code/@code="33999-4"]/value³|.clinicalStatus|[CDA Allergy Status Observation value → FHIR clinicalStatus](./ConceptMap-CF-AllergyStatus.html)
...
The example below is generated from the canonical Myra Jones CCD (aspirin allergy concern) and the published FHIR example instance, and is verified against them on every build. Note: per the IG's own type ConceptMap, 419511003 has no `type` mapping, so the resource carries category and criticality but no type. The section also contains a peanut (food) allergy entry available for future variety.

{% include examples/CF-allergy-example.xhtml %}
