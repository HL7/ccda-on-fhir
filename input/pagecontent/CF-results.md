...
The example below is generated from the canonical Myra Jones CCD (urinalysis organizer with a specific-gravity component) and the published FHIR example instances, verified on every build. It demonstrates all six USCDI-3 Laboratory elements: test, value, interpretation, reference range, status, and specimen type.

{% include examples/CF-result-example.xhtml %}
