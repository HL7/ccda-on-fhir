...
The example below is generated from the canonical Myra Jones CCD and the published FHIR example instances, and is verified against them on every build (values shown are checked pairs; see the project decision log for content decisions). The medication is represented as a referenced Medication resource; the commented block illustrates the medicationCodeableConcept alternative.

{% include examples/CF-medication-example.xhtml %}

#### Links to example content

* [CDA Input entry](./Binary-CDA-Input-MedicationRequest.html) (from the Myra Jones master CCD)
* [MedicationRequest example](./MedicationRequest-FHIR-Output-MedicationRequest.html)
* [Medication example](./Medication-FHIR-Output-Medication.html)