...
The example below is generated from the canonical Myra Jones CCD (smoking status observation) and the published FHIR example instance, verified on every build. The section's other entries (tribal affiliation, sexual orientation, gender identity, pregnancy status, disability status) are candidates for further examples per the USCDI-3 coverage list.

{% include examples/CF-social-example.xhtml %}
